...
var SCRIPT_ROLE = 'ANALYTICS';
var ANALYTICS_GET_ACTIONS = {
  getLeaderboard: true,
...
var ANALYTICS_META_ACTIONS = {
  FORCE_SERVER_REPULL: true
};
var ROLE_GET_ACTIONS = SCRIPT_ROLE === 'CORE' ? CORE_GET_ACTIONS : ANALYTICS_GET_ACTIONS;
var ROLE_META_ACTIONS = SCRIPT_ROLE === 'CORE' ? CORE_META_ACTIONS : ANALYTICS_META_ACTIONS;

function isActionAllowedForScript_(action) {
  var key = safeStr(action);
  return !!ROLE_GET_ACTIONS[key];
}

function isMetaActionAllowedForScript_(metaAction) {
  var key = safeStr(metaAction).toUpperCase();
  return !!ROLE_META_ACTIONS[key];
}
...
function doGet(e) {
  try {
    ...
    if (!isActionAllowedForScript_(action)) {
      return jsonOut({ success: false, error: 'Action not supported by ' + SCRIPT_ROLE + ' script: ' + action });
    }
...
function doPost(e) {
  try {
    ...
    if (!isMetaActionAllowedForScript_(metaAction)) {
      return jsonOut({ success: false, error: 'metaAction not supported by ' + SCRIPT_ROLE + ' script: ' + (metaAction || '(submit)') });
    }
